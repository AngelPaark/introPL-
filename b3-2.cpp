...
/**
 * \brief Пользовательский ввод точности вычисления
 * \param message Мотивационное сообщение
 * \return Точность вычисления
 */
double get_eps(const std::string& message);

/**
 * \brief Рассчитывает сумму последовательность с точностью
 * \param eps Точность вычисления
 * \return Сумма последовательности
 */
double get_summ(const double eps);
...
int main() noexcept
{
    try
    {
        ...
        const auto eps = get_eps("Введите точность вычислений ");
        const double summ = get_summ(eps);
